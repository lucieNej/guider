...
self.addEventListener('fetch', evt => {
    ...
    evt.respondWith(
        fetch(evt.request).then(res => {
            ...
            return res.clone(); //on va chercher si c'est ds le cache, si ce n'est pas le cas, on cherche ds reseau et avec put on le met dans le cache 
        }) // clone : on ne peut lire l'info qu'une fois, c'est pkoi on clone 
